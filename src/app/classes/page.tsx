  ...
  return (
    <AppShell active="/classes" userName={user.name ?? user.email}>
      ...
      <p className="mb-6 text-sm text-ink-soft">
        Dedicated pages per class — overview, assignments, notes, exams, resources, and a
        class-scoped AI assistant — are coming in phase 2. For now, here's what's synced.
      </p>
      ...
      {classes.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {classes.map((c) => (
            <div key={c.id} className="rounded-xl2 border border-border-soft bg-surface p-4 shadow-card">
